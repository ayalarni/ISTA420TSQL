...
--2
select p.productid, p.productname from products p where p.categoryid in
...
--5
select p.productname, p.productid from products p where p.supplierid in
 (select s.supplierid from suppliers s where s.country in ('Japan', 'Singapore', 'Australia'));
 date of sale and employee select o.orderdate, o.employeeid from orders o where o.orderdate =
 (select max(o1.orderdate) from orders o1 where o1.employeeid = o.employeeid )order by o.employeeid;

--6
select o.orderdate, o.employeeid from orders o where o.orderdate =
 (select min(o1.orderdate) from orders o1 where o1.employeeid = o.employeeid )order by o.employeeid;
